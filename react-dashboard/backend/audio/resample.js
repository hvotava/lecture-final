...
const { muLawToPCM16, pcm16ToMuLaw } = require('./mulaw');
 ...
 * @param {Int16Array} input PCM16 data
 * @param {number[]} coeffs Filter koeficienty
 * @returns {Int16Array} Filtrovaná data
 */
function applyFIRFilter(input, coeffs) {
 ...
 * @param {Int16Array} input PCM16 @ 8kHz
 * @returns {Int16Array} PCM16 @ 24kHz
 */
function upsample8kTo24k(input) {
 ...
 * @param {Int16Array} input PCM16 @ 24kHz
 * @returns {Int16Array} PCM16 @ 8kHz
 */
function downsample24kTo8k(input) {
  if (input.length === 0) return new Int16Array(0);
 ...
 * @param {Int16Array} input PCM16 data
 * @param {number} ratio Poměr výstupní/vstupní sample rate
 * @returns {Int16Array} Resamplovaná data
 */
 ...
 * @param {Int16Array} input PCM16 data
 * @returns {Int16Array} Data bez DC offsetu
 */
 ...
 * @param {Buffer} mulawData μ-law buffer @ 8kHz
 * @returns {Int16Array} PCM16 @ 24kHz
 */
function convertTwilioToOpenAI(mulawData) {
  // μ-law → PCM16 @ 8kHz
 ...
 * @param {Int16Array} pcm24k PCM16 @ 24kHz
 * @returns {Buffer} μ-law buffer @ 8kHz
 */
function convertOpenAIToTwilio(pcm24k) {
  // 24kHz → 8kHz
