 ...
 * @param ws WebSocket connection k OpenAI
 * @param pcmInt16LE PCM16 data @ 24kHz
 */
export function sendAppendPCM(ws: WebSocket, pcmInt16LE: Int16Array): void {
 ...
 * @param ws WebSocket connection k OpenAI
 */
export function sendCommit(ws: WebSocket): void {
  if (ws.readyState !== WebSocket.OPEN) {
 ...
 * @param ws WebSocket connection k OpenAI
 * @param options Volitelné parametry
 */
export function sendCreateResponse(ws: WebSocket, options?: {
  instructions?: string;
  voice?: string;
  temperature?: number;
  ...
  Object.keys(event.response).forEach(key => {
    if (event.response[key as keyof typeof event.response] === undefined) {
      delete event.response[key as keyof typeof event.response];
    }
  });
 ...
 * @param ws WebSocket connection k OpenAI
 * @param content Text obsah zprávy
 * @param role Role ('user' nebo 'assistant')
 */
export function sendConversationItem(ws: WebSocket, content: string, role: 'user' | 'assistant' = 'user'): void {
  if (ws.readyState !== WebSocket.OPEN) {
 ...
 * @param ws WebSocket connection k OpenAI
 */
export function sendCancelResponse(ws: WebSocket): void {
  if (ws.readyState !== WebSocket.OPEN) {
 ...
 * @param ws WebSocket connection k OpenAI
 * @param updates Aktualizace session
 */
export function sendSessionUpdate(ws: WebSocket, updates: {
  instructions?: string;
  voice?: string;
  temperature?: number;
  turn_detection?: any;
 ...
 * @returns Parsed event nebo null
 */
export function parseOpenAIEvent(data: WebSocket.Data): any | null {
  try {
 ...
 */
export function logOpenAIEvent(event: any, prefix: string = '📥'): void {
