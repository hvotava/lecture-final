...
// Audio processing
import { base64MuLawToPCM16, pcm16ToBase64MuLaw } from '../audio/mulaw';
import { convertTwilioToOpenAI, convertOpenAIToTwilio } from '../audio/resample';
...
export function setupTwilioWebSocket(server: Server): void {
  ...
  // Handle WebSocket upgrade pro /twilio/stream
  server.on('upgrade', (request, socket, head) => {
    const { pathname } = parse(request.url || '');
    
    if (pathname === '/twilio/stream') {
      console.log('🔄 WebSocket upgrade request for /twilio/stream');
  ...
  wss.on('connection', (twilioWs, request) => {
     ...
     */
    async function connectToOpenAI(): Promise<void> {
      try {
     ...
     */
    function processAudioFromTwilio(base64Audio: string): void {
  ...
  console.log('🎧 WebSocket server ready for /twilio/stream');
} 